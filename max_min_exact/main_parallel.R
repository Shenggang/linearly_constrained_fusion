...
total_steps=3
results = list()
results = foreach(idx=1:48, .packages = c("R6","statmod")) %dopar%
{
  cat('=============Slot ', idx, '===============\n')
  ...
  parameters = list(alpha=phi[, slots], glg_params=glg_params[slots], intercept=intercept[slots], beta=beta[, slots],
                    order=order, consMat=consMat, consY=constraints[idx, ]*scaling, time_pool=time_pool)
  imputed = impute_energy(total_samples, total_steps, per_steps,
                          ...
                          max_trial=100, positive=TRUE,idx=idx)
...
save(results, file=paste('result_n', total_samples, '_d', test_days, '+', total_steps,  
